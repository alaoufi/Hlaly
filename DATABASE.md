...
| `mrahi_external_sires` | فحول خارج المراح (نوع الحلال، رقم/اسم، صاحبه وجواله، الأب، ملاحظات) — سجل مرجعي فقط، ليست بهائم. `type` غائب في سجلات أُضيفت قبل هذا الحقل — تظهر في قائمة اختيار الأب لكل الأنواع احتياطاً حتى تُعدَّل |
