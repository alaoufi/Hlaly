...
> رقم النسخة يتغيّر تلقائياً مع كل بناء (`1.0.<رقم تشغيل GitHub Actions>`) —
> للاطّلاع على آخر نسخة منشورة راجع `version.json` على وسم الإصدار `apk-latest`
> في المستودع، أو تبويب Actions. انظر قسم «الإصدار والنشر» أدناه.
...
- **`updater.js`** — بوّابة تحديث إلزامية: تفحص عند الإقلاع وعند عودة التطبيق للواجهة (بلا حجب مطلقاً دون إنترنت)، وتُنزِّل وتثبِّت التحديث عبر إضافة `mrahi-updater` الأصلية (القسم أدناه).
- **`plugins/`** — إضافات Capacitor محلية بكود أندرويد أصلي (Java)، كل واحدة حزمة npm محلية (`file:plugins/<name>`):
  - `mrahi-save-folder` — اختيار مجلد حفظ دائم (SAF) للنسخ الاحتياطية وملفات التبادل.
  - `mrahi-updater` — تنزيل وتثبيت تحديث APK محلياً (`Filesystem.downloadFile` إلى مساحة التطبيق الخاصة، ثم `FileProvider` + نيّة تثبيت أصلية) — بلا اعتماد على مدير تنزيلات النظام.
...
3. **قائمة/تبويب:** بند في `quickMenuCats()` (قائمة ⋮ الأدوات والعمليات) أو
   `settingsMenuCats()` (قائمة ☰ الإعدادات والإدارة)، أو أيقونة في `buildNav()`
   لتبويب رئيسي بالشريط السفلي.
...
- **`release.keystore` مُضمَّن فعلياً في هذه الحزمة** (انظر قسم «مفتاح التوقيع»
  أعلاه) لأن CI يحتاجه مباشرة من المستودع. عاملْه كسرّ رغم وجوده في الكود —
  لا تُشاركه مع أي طرف لا تثق به، فمن يملكه ينشر تحديثات تُثبَّت فوق نسخ
  المستخدمين الحاليين. مفتاح تفعيل التراخيص (Ed25519) في `license.js` منفصل
  تماماً عن هذا المفتاح.
- الأدلّة تحت `docs/` وداخل التطبيق (`guide.js` — زر «دليل الاستخدام» بالتطبيق).
- **`DATABASE.md`** يوثّق كل متاجر IndexedDB وحقولها ومفاتيح localStorage
  بالتفصيل — راجعه قبل إضافة حقل أو متجر جديد لتتبع نفس النمط القائم.
