...
function screenHome() {
  ...
  view().querySelectorAll('[data-born]').forEach(c => c.addEventListener('click', () => { animalFilter = ''; animalStatusSel = ['present']; animalSourceSel = ['born']; animalSexSel = c.dataset.born === 'all' ? [] : [c.dataset.born]; saveAnimalFilters(); setHash('#/animals'); }));
...
function screenAnimals() {
  ...
  const cb = (on) => (on ? '☑' : '☐') + ' ';
  const stChips = `<div class="chips"><span class="chip ${!animalStatusSel.length ? 'active' : ''}" data-s="">الكل</span><span class="chip ${animalStatusSel.includes('present') ? 'active' : ''}" data-s="present">${cb(animalStatusSel.includes('present'))}في الحظيرة</span><span class="chip ${animalStatusSel.includes('sold') ? 'active' : ''}" data-s="sold">${cb(animalStatusSel.includes('sold'))}مباعة</span><span class="chip ${animalStatusSel.includes('dead') ? 'active' : ''}" data-s="dead">${cb(animalStatusSel.includes('dead'))}نافقة</span></div>`;
  const srcChips = `<div class="chips"><span class="chip ${!animalSourceSel.length ? 'active' : ''}" data-src="">كل المصادر</span><span class="chip ${animalSourceSel.includes('born') ? 'active' : ''}" data-src="born">${cb(animalSourceSel.includes('born'))}👶 مواليد</span><span class="chip ${animalSourceSel.includes('purchased') ? 'active' : ''}" data-src="purchased">${cb(animalSourceSel.includes('purchased'))}🛒 مشترى</span><span class="chip ${animalSourceSel.includes('sale') ? 'active' : ''}" data-src="sale">${cb(animalSourceSel.includes('sale'))}💰 للبيع (المعدّ للبيع)</span></div>`;
  const sexChips = `<div class="chips"><span class="chip ${!animalSexSel.length ? 'active' : ''}" data-sex="">الجنسان</span>${SEX.map(s => `<span class="chip ${animalSexSel.includes(s.k) ? 'active' : ''}" data-sex="${s.k}">${cb(animalSexSel.includes(s.k))}${s.k === 'male' ? '♂ ' : '♀ '}${s.ar}</span>`).join('')}</div>`;
...
function screenAnimalDetail(arg) {
  ...
  view().innerHTML = summary + `
    <div class="card"><h3>البيانات الأساسية</h3>
      ...
      ${(a.source === 'purchased' && a.buy_price != null) ? row('💵 سعر الشراء', esc(String(a.buy_price))) : ''}
