...
// عمر حدّ البلوغ (احتساب المولود) في الحظيرة (لكل نوع، بالأشهر): أصغر منه «يتبع أمّه» ولا يُعدّ في «في الحظيرة» (لكن يبقى ظاهراً)
...
// قاعدة الاحتساب لكل نوع: { age: أشهر, sex: 'both'|'male'|'female', malePenId, femalePenId: حظيرة الوجهة عند بلوغ العمر (اختياري) } (تدعم القيمة القديمة كرقم)
function countRuleFor(type) {
  const v = loadCountAge()[type];
  if (v == null) return { mode: 'age', age: 0, sex: 'both', malePenId: null, femalePenId: null };
  if (typeof v === 'number') return { mode: 'age', age: v > 0 ? v : 0, sex: 'both', malePenId: null, femalePenId: null };
  const age = parseInt(v.age, 10);
  return {
    mode: v.mode === 'manual' ? 'manual' : 'age',
    age: age > 0 ? age : 0,
    sex: (v.sex === 'male' || v.sex === 'female') ? v.sex : 'both',
    malePenId: v.malePenId != null ? v.malePenId : null,
...
// الفحل لا يمكن أن يكون من نوع حلال غير نوع البهيمة (لا فحل ماعز لأب إبل مثلاً) — type يحصر الخيارات بنوع واحد فقط.
// الفحول الخارجية القديمة المُضافة قبل وجود حقل النوع (type غائب) تبقى ظاهرة لكل الأنواع احتياطاً حتى تُعدَّل يدوياً.
function sireOptionsInner(type, placeholder) {
  const sires = siresList().filter(s => !type || s.type === type);
  const ext = loadExternalSires().filter(s => !type || !s.type || s.type === type);
...
function sireSelectHtml(id, placeholder, type) {
  const inner = sireOptionsInner(type, placeholder);
  return `<div class="field${inner ? '' : ' hidden'}" id="${id}_wrap">${inner ? `<select id="${id}">${inner}</select>` : ''}</div>`;
}
// يعيد بناء قائمة الفحول عند تغيّر نوع الحلال (مثلاً في نموذج إضافة/تعديل بهيمة) — onRebind تُعاد فيها ربط أحداث التغيير لأن العنصر استُبدل
function rebuildSireSelect(id, type, placeholder, onRebind) {
  const wrap = document.getElementById(id + '_wrap'); if (!wrap) return;
  const inner = sireOptionsInner(type, placeholder);
  wrap.classList.toggle('hidden', !inner);
  wrap.innerHTML = inner ? `<select id="${id}">${inner}</select>` : '';
  if (inner && onRebind) onRebind();
}
...
async function loadAll() {
  ...
  await migratePensToIds(); // ترحيل الحظائر من الاعتماد على الاسم إلى رقم داخلي ثابت (مرة واحدة)
...
function screenHome() {
  ...
  view().querySelectorAll('[data-sfilter]').forEach(c => c.addEventListener('click', () => {
    const s = c.dataset.sfilter;
    if (s === 'present') { animalFilter = ''; animalSourceSel = ALL_SOURCES.slice(); animalSexSel = ALL_SEXES.slice(); saveAnimalFilters(); pendingCountedOnly = true; setHash('#/animals'); return; }
    // مباعة/نافقة انتقلتا إلى الأرشيف — لم تعودا تظهران في «🐑 الحلال» إطلاقاً
    pendingArchiveStatus = s; setHash('#/archive');
  }));
...
// بهائم بلغت حظيرة الوجهة المُعدَّة لها (سواء ببلوغ عمر حدّ البلوغ، أو بضبط غرض الذكر «معدّ للبيع») ولم تُنقل إليها بعد —
// اقتراح فقط يحتاج تأكيدك (زر «نقل الآن»)؛ لا نقل صامت. «تجاهل» يُسكِت الاقتراح لهذه الوجهة بعينها دون منع اقتراحات لاحقة.
function penTransferCandidates() {
  const out = [];
  (C.animals || []).forEach(a => {
    if (a.status !== 'present' || !a.type) return;
    const r = countRuleFor(a.type);
    const destId = a.sex === 'male' ? r.malePenId : (a.sex === 'female' ? r.femalePenId : null);
    if (destId == null || a.pen_id === destId) return;
    const ageReady = r.mode === 'age' && r.age > 0 && a.birth && ageMonths(a.birth) >= r.age;
    const saleReady = a.sex === 'male' && a.purpose === 'sale';
    if (!ageReady && !saleReady) return;
    if (a.pen_transfer_skip === destId) return;   // جرى تجاهله لهذه الوجهة بعينها
    out.push({ a, destId, reason: saleReady ? 'sale' : 'age' });
  });
  return out;
}
...
function screenAlerts() {
  ...
  const remCards = rems.map(({ r, mt }) => `<div class="card" style="background:#ede7f6"><h3>🔔 ${esc(r.title || 'تنبيه')} <span class="muted" style="font-weight:400">(${mt.length})</span></h3>${mt.map(a => `<div class="row click" data-goa="${a.id}"><span class="k">${display(a)}</span><span class="v">${esc(sexTerm(a))}${a.birth ? ' • ' + (ageText(a.birth) || '') : ''}${animalPenName(a) ? ' • 🏠 ' + esc(animalPenName(a)) : ''}</span></div>`).join('')}</div>`).join('');
  const transferCard = transfers.length ? `<div class="card" style="background:#ede7f6"><h3>🔀 بهائم جاهزة للنقل لحظيرة (${transfers.length})</h3>${transfers.map(({ a, destId, reason }) => {
    const destName = (penById(destId) || {}).name || '';
    return `<div class="row" style="align-items:center;flex-wrap:wrap;gap:6px">
      <span class="k click" data-goa="${a.id}">${display(a)} <span class="muted" style="font-weight:400;font-size:.78rem">${reason === 'sale' ? '💰 معدّ للبيع' : '🌱 بلغ عمر حدّ البلوغ'} → ${esc(destName)}</span></span>
      <span style="display:flex;gap:6px"><button class="btn sm" data-pxgo="${a.id}|${destId}">🔀 نقل الآن</button><button class="btn sm outline" data-pxskip="${a.id}|${destId}">تجاهل</button></span></div>`;
  }).join('')}</div>` : '';
  view().innerHTML = remCards + transferCard + `
  ...
  view().querySelectorAll('[data-pxgo]').forEach(b => b.addEventListener('click', async () => {
    const [id, destId] = b.dataset.pxgo.split('|').map(n => parseInt(n, 10));
    const ok = await guard(async () => { await dbUpdate('animals', id, { pen_id: destId, pen_transfer_skip: null }); });
    if (ok) { toast('تم النقل'); await loadAll(); screenAlerts(); }
  }));
  view().querySelectorAll('[data-pxskip]').forEach(b => b.addEventListener('click', async () => {
    const [id, destId] = b.dataset.pxskip.split('|').map(n => parseInt(n, 10));
...
function animalCard(a) {
  ...
  return `<div class="card click" data-aid="${a.id}">
    ...
    ${animalPenName(a) ? `<div class="li-sub">🏠 ${esc(animalPenName(a))}</div>` : ''}
...
function animalListModal(title, list) {
  openModal(title,
    ...
    + (list.length ? list.map(o => `<div class="card click" data-aid="${o.id}" style="margin:6px 0"><div class="li-title">${display(o)}</div><div class="li-sub">${esc(sexTerm(o))}${o.birth ? ' • ' + fmtDate(o.birth) : ''}${animalPenName(o) ? ' • ' + esc(animalPenName(o)) : ''}</div></div>`).join('') : noItem()),
...
function screenAnimals() {
  ...
  if (pendingNewbornFilter !== null) {
    animalFilter = ''; animalSourceSel = ['born']; animalSexSel = pendingNewbornFilter === 'all' ? ALL_SEXES.slice() : [pendingNewbornFilter];
  ...
  const penChip = `<div class="chips"><span class="chip ${noPenOnly ? 'active' : ''}" data-nopen="1">${cb(noPenOnly)}🏠 بدون حظيرة فقط</span></div>`;
...
/* ===== الأرشيف: الحلال الذي خرج من الحظيرة (بيع/نفوق/اهداء/فقد/ذبح) ===== */
// خمس حالات الخروج تنتقل هنا مباشرة (تلقائياً بمجرّد تسجيل الخروج) ولا تظهر إطلاقاً في 🐑 الحلال ولا 🐏 الفحول ولا 👶 المواليد
// ولا في بحث الرئيسية — مكانها الوحيد هذه الشاشة، بانتظار «↩ إعادة للحظيرة» إن كان الخروج بالخطأ، أو «🗑️ حذف نهائي».
const ARCHIVE_STATUSES = ['sold', 'dead', 'given', 'missing', 'slaughtered'];
let archiveStatusFilter = '';   // '' = الكل، أو إحدى ARCHIVE_STATUSES
let pendingArchiveStatus = null;   // طلب لمرة واحدة من بطاقات الرئيسية (مباعة/نافقة)
function screenArchive() {
  if (!can('animals', 'view')) { view().innerHTML = noPerm(); return; }
  if (pendingArchiveStatus !== null) { archiveStatusFilter = pendingArchiveStatus; pendingArchiveStatus = null; }
  const canEdit = can('animals', 'edit');
  const all = C.animals.filter(a => ARCHIVE_STATUSES.includes(a.status) && (!animalFilter || a.type === animalFilter));
  const list = sortAnimals(archiveStatusFilter ? all.filter(a => a.status === archiveStatusFilter) : all);
  const statChip = (k, label) => `<span class="chip ${archiveStatusFilter === k ? 'active' : ''}" data-af="${k}">${label} (${all.filter(a => !k || a.status === k).length})</span>`;
  const exitDateOf = (a) => a.sale_date || a.dead_date || a.gift_date || a.missing_date || a.slaughter_date;
  const card = (a) => {
    const ic = STATUS_ICON[a.status] || '';
    const d = exitDateOf(a);
    return `<div class="card" style="margin:6px 0">
      <div class="li-title click" data-aid="${a.id}">${display(a)}</div>
      <div class="li-sub">${arOf(TYPES, a.type)} • ${esc(sexTerm(a))} • <span class="badge">${ic} ${arOf(STATUS, a.status)}</span>${d ? ' • ' + fmtDate(d) : ''}</div>
      ${canEdit ? `<div class="btn-row" style="margin-top:6px">
        <button class="btn sm outline" data-arback="${a.id}">↩ إعادة للحظيرة</button>
        <button class="btn sm danger" data-ardel="${a.id}">🗑️ حذف نهائي</button>
      </div>` : ''}</div>`;
  };
  view().innerHTML = typeChipsHtml() + `<div class="muted" style="margin:8px 0">الحلال الذي خرج من الحظيرة — لا يظهر في 🐑 الحلال ولا 🐏 الفحول ولا 👶 المواليد ولا الرئيسية. أعِده للحظيرة إن كان الخروج بالخطأ، أو احذفه نهائياً (يُنقل إلى 🗑️ سلة المحذوفات، يمكن استعادته خلال ٣٠ يوماً).</div>
    <div class="chips">${statChip('', 'الكل')}${statChip('sold', '💰 مباعة')}${statChip('dead', '📉 نافقة')}${statChip('given', '🎁 اهداء')}${statChip('missing', '🔎 مفقودة')}${statChip('slaughtered', '🔪 مذبوحة')}</div>
    <div class="muted" style="margin:8px 0">العدد: ${list.length}</div>
    ${list.length ? list.map(card).join('') : '<div class="center-empty">الأرشيف فارغ.</div>'}`;
  bindTypeChips(screenArchive);
  view().querySelectorAll('[data-af]').forEach(c => c.addEventListener('click', () => { archiveStatusFilter = c.dataset.af; screenArchive(); }));
  view().querySelectorAll('[data-aid]').forEach(el => el.addEventListener('click', () => setHash('#/animal/' + el.dataset.aid)));
  view().querySelectorAll('[data-arback]').forEach(b => b.addEventListener('click', async (e) => {
    e.stopPropagation();
    const a = animalById(parseInt(b.dataset.arback, 10)); if (a) await quickRevert(a, screenArchive);
  }));
  view().querySelectorAll('[data-ardel]').forEach(b => b.addEventListener('click', async (e) => {
    e.stopPropagation();
    const id = parseInt(b.dataset.ardel, 10);
    if (!await confirm2('حذف نهائي لهذه البهيمة؟ تُنقل إلى 🗑️ سلة المحذوفات (يمكن استعادتها خلال ٣٠ يوماً).', { danger: true })) return;
    const ok = await guard(async () => { await dbDelete('animals', id); });
    if (ok) { toast('تم الحذف'); await loadAll(); screenArchive(); }
  }));
}
...
function nextPenId() {
  let seq = 0; try { seq = parseInt(localStorage.getItem('mrahi_pen_id_seq') || '0', 10) || 0; } catch (e) { /* تجاهل */ }
  seq += 1;
  try { localStorage.setItem('mrahi_pen_id_seq', String(seq)); } catch (e) { /* تجاهل */ }
  return seq;
}
function addPen(name, type, parentId) {
  name = String(name || '').trim(); if (!name) return null;
  const l = loadPens();
  const existing = l.find(p => p.name === name);
  if (existing) return existing.id;
  const pen = { id: nextPenId(), name, type: type || '', parentId: parentId != null ? parentId : null };
  l.push(pen); savePens(l);
  return pen.id;
}
// حظيرة نظام تلقائية «معدّ للبيع» لكل نوع — تُنشأ مرة واحدة عند الحاجة (بلا تدخّل يدوي)، وتظهر في شاشة إدارة
// الحظائر فقط أثناء وجود بهائم عليها فعلياً (تختفي تلقائياً من القائمة عند خلوّها — لكنها تبقى خياراً متاحاً دائماً).
function ensureSalePen(typeKey) {
  const l = loadPens();
  const ex = l.find(p => p.system === 'sale' && p.type === (typeKey || ''));
  if (ex) return ex.id;
  const pen = { id: nextPenId(), name: '💰 معدّ للبيع', type: typeKey || '', parentId: null, system: 'sale' };
  l.push(pen); savePens(l);
  return pen.id;
}
function penById(id) { if (id == null || id === '') return null; const n = typeof id === 'number' ? id : parseInt(id, 10); return allPens().find(p => p.id === n) || null; }
// الحظائر الرئيسية (بلا أب) لنوعٍ معيّن — تُستخدم كخيارات «تنتمي إلى» عند إنشاء فرع
function rootPensForType(typeKey) { return allPens().filter(p => p.parentId == null && (!p.type || p.type === typeKey)); }
function penChildren(id) { return allPens().filter(p => p.parentId === id); }
// اسم حظيرة البهيمة الحالي (يُقرأ دائماً من تعريف الحظيرة عبر رقمها — لا يوجد نص مخزَّن على البهيمة نفسها)
function animalPenName(a) { const p = a && a.pen_id != null ? penById(a.pen_id) : null; return p ? p.name : ''; }
// خيارات حقل الحظيرة: كل فرع نصّه يذكر اسم رئيسيته صراحةً داخل نص الخيار نفسه (بدل الاعتماد على optgroup فقط) —
// يضمن ظهورها بوضوح مهما اختلف عرض قائمة الاختيار الأصلية على الجهاز. القيمة المخزَّنة في كل <option> هي رقم الحظيرة.
function penOptions(selectedId, typeKey) {
  const sel = selectedId != null && selectedId !== '' ? (typeof selectedId === 'number' ? selectedId : parseInt(selectedId, 10)) : null;
  const defs = allPens().filter(p => !p.type || p.type === typeKey);
  const roots = defs.filter(p => p.parentId == null).sort((a, b) => a.name.localeCompare(b.name, 'ar'));
  const seen = new Set();
  let opts = '';
  roots.forEach(r => {
    seen.add(r.id);
    opts += `<option value="${r.id}" ${r.id === sel ? 'selected' : ''}>🏠 ${esc(r.name)}</option>`;
    const kids = defs.filter(c => c.parentId === r.id).sort((a, b) => a.name.localeCompare(b.name, 'ar'));
    kids.forEach(k => { seen.add(k.id); opts += `<option value="${k.id}" ${k.id === sel ? 'selected' : ''}>${esc(r.name)} ↳ ${esc(k.name)}</option>`; });
  });
  // فروع أبوها من نوع حلال آخر (نادر) — تُعرض احتياطاً حتى لا تختفي
  defs.filter(p => p.parentId != null && !seen.has(p.id)).forEach(p => { seen.add(p.id); const parentName = (penById(p.parentId) || {}).name || '؟'; opts += `<option value="${p.id}" ${p.id === sel ? 'selected' : ''}>${esc(parentName)} ↳ ${esc(p.name)}</option>`; });
  if (sel != null && !seen.has(sel)) { const stalePen = penById(sel); if (stalePen) opts = `<option value="${sel}" selected>${esc(stalePen.name)}</option>` + opts; }
...
function penField(id, selectedId, typeKey) {
  return `<div class="field"><select id="${id}">${penOptions(selectedId, typeKey)}</select>
...
function rebuildPen(penId, typeKey) { const s = document.getElementById(penId); if (!s) return; s.innerHTML = penOptions(null, typeKey); bindPenField(penId); }
// رقم الحظيرة المختارة (أو إنشاء حظيرة جديدة بالاسم المكتوب وإرجاع رقمها الجديد)
function penValue(id, typeKey) {
  const s = val(id);
  if (s === '__new__') { const v = val(id + '_new').trim(); return v ? addPen(v, typeKey || '', null) : null; }
  return s ? parseInt(s, 10) : null;
}
// ترحيل لمرة واحدة: الصيغة القديمة للحظائر كانت تُعرَّف بالاسم (وتُربط بالبهيمة عبر a.pen النصّي)، وأي تعديل للاسم
// يتطلّب تحديث كل بهيمة مرتبطة به — وهذا ما كان يفشل بصمت عند قفل التعديل. الصيغة الجديدة تُسند لكل حظيرة رقماً
// داخلياً ثابتاً (id) لا يتغيّر أبداً، وتربط البهيمة به عبر a.pen_id — فتعديل الاسم يظهر فوراً في كل مكان دون أي تحديث جماعي.
async function migratePensToIds() {
  let done = false; try { done = !!localStorage.getItem('mrahi_pens_id_migrated'); } catch (e) { /* تجاهل */ }
  if (done) return;
  try {
    let raw = []; try { raw = JSON.parse(localStorage.getItem('mrahi_pens') || '[]'); } catch (e) { raw = []; }
    const normalized = raw.map(p => typeof p === 'string' ? { name: p.trim(), type: '', parentName: '' } : { name: String(p.name == null ? '' : p.name).trim(), type: p.type || '', parentName: String(p.parent || '').trim() }).filter(p => p.name);
    (C.animals || []).forEach(a => {
      const nm = String(a.pen == null ? '' : a.pen).trim(); if (!nm) return;
      if (!normalized.some(p => p.name === nm)) normalized.push({ name: nm, type: a.type || '', parentName: '' });
    });
    const uniq = []; const seenNames = new Set();
    normalized.forEach(p => { if (!seenNames.has(p.name)) { seenNames.add(p.name); uniq.push(p); } });
    const withIds = uniq.map((p, i) => ({ id: i + 1, name: p.name, type: p.type, parentName: p.parentName }));
    const byName = {}; withIds.forEach(p => { byName[p.name] = p; });
    const finalPens = withIds.map(p => ({ id: p.id, name: p.name, type: p.type, parentId: (p.parentName && byName[p.parentName]) ? byName[p.parentName].id : null }));
    savePens(finalPens);
    try { localStorage.setItem('mrahi_pen_id_seq', String(finalPens.length)); } catch (e) { /* تجاهل */ }
    const byNameFinal = {}; finalPens.forEach(p => { byNameFinal[p.name] = p.id; });
    for (const a of (C.animals || [])) {
      const nm = String(a.pen == null ? '' : a.pen).trim();
      if (!nm || a.pen_id != null) continue;
      const pid = byNameFinal[nm];
      if (pid != null) { try { await dbUpdate('animals', a.id, { pen_id: pid }, true); a.pen_id = pid; } catch (e) { /* أفضل جهد */ } }
    }
    try { localStorage.setItem('mrahi_pens_id_migrated', '1'); } catch (e) { /* تجاهل */ }
  } catch (e) { /* أفضل جهد — لا نكسر الإقلاع بسبب الترحيل */ }
}
...
function screenAnimalEdit(arg) {
  ...
  view().innerHTML = `
    ...
    <div class="card"><h3>النسب</h3>
      ...
      ${sireSelectHtml('f_fatherSel', '— يختار الأب من داخل أو خارج الحظيرة، أو اترك فارغاً واكتب يدوياً —', a ? a.type : (animalFilter || 'sheep'))}
  ...
  document.getElementById('f_type').addEventListener('change', () => {
    rebuildPen('f_pen', val('f_type'));   // حظائر النوع المحدّد فقط
    // فحول النوع المحدّد فقط — لا يصحّ أب من نوع حلال آخر (ماعز/إبل/غنم...)
    rebuildSireSelect('f_fatherSel', val('f_type'), '— يختار الأب من داخل أو خارج الحظيرة، أو اترك فارغاً واكتب يدوياً —', () => bindSireSelectSingle('f_fatherSel', 'f_father'));
  });
  ...
  { const cl = document.getElementById('cloneLast'); if (cl) cl.addEventListener('click', () => { const L = lastAnimal || {}; setVal('f_type', L.type); setVal('f_kind', L.idkind); setVal('f_sex', L.sex); setVal('f_source', L.source); setVal('f_color', L.color); setVal('f_tagcolor', L.tag_color || ''); setVal('f_tagshape', L.tag_shape || ''); setVal('f_father', L.father_name || ''); rebuildPen('f_pen', L.type || (animalFilter || 'sheep')); setVal('f_pen', L.pen_id != null ? String(L.pen_id) : ''); syncKind(); toast('نُسخت بيانات آخر إدخال'); }); }
  document.getElementById('saveBtn').addEventListener('click', async () => {
    ...
    const ok = await guard(async () => {
      ...
      if (obj.source === 'purchased') {
        ...
        if (ho && ho.checked) {
          ...
          const offObj = (sex) => ({ type: obj.type, pen_id: obj.pen_id, idkind: 'number', code: '', name: '', sex, purpose: '', source: 'purchased', designation: '', buy_price: null, birth: null, color: '', status: 'present', mother_id: inserted.id, father_name: '', notes: 'مولود رافق أمّه عند الشراء' });
    ...
    if (ok) {
      ...
      if (!a) {
        lastPen = obj.pen_id; try { localStorage.setItem('mrahi_last_pen', lastPen == null ? '' : String(lastPen)); } catch (e) {}
        lastAnimal = { type: obj.type, pen_id: obj.pen_id, idkind: obj.idkind, sex: obj.sex, source: obj.source, color: obj.color, tag_color: obj.tag_color, tag_shape: obj.tag_shape, father_name: obj.father_name };
...
function screenAnimalDetail(arg) {
  ...
  REC.basic = `<div class="card"><h3>📋 البيانات الأساسية</h3>
      ...
      ${row('🏠 المكان', esc(animalPenName(a)) || '—')}
...
function addOffspringModal(mother) {
  openModal('مواليد ' + display(mother), `
    ...
    ${penField('of_pen', mother.pen_id, mother.type)}
    <div id="ofSingle">
    ...
    document.getElementById('of_save').addEventListener('click', async () => {
      ...
      const base = { type: mother.type, pen_id: pen, source: 'born', status: 'present', mother_id: mother.id, father_name: '', notes: '' };
...
function screenMating(arg) {
  ...
  view().innerHTML = `<div class="card"><h3>سجل التلقيح</h3>
    ...
    ${sireSelectHtml('m_sireSel', null, preset ? preset.type : null)}
...
function matingEditModal(m) {
  ...
  const matedAnimal = animalById(m.animal_id);
  openModal('تعديل التلقيح', `
    ${fInput('تاريخ التلقيح', 'me_date', m.date, 'date')}
    ${sireSelectHtml('me_sireSel', null, matedAnimal ? matedAnimal.type : null)}
...
function startPregBulkModal() {
  ...
  const pens = [...new Set(all.map(a => a.pen_id).filter(id => id != null))].map(id => penById(id)).filter(Boolean).sort((a, b) => a.name.localeCompare(b.name, 'ar'));
  ...
  const penChips = pens.length > 1 ? `🏠 <div class="chips"><span class="chip active" data-penf="">الكل</span>${pens.map(p => `<span class="chip" data-penf="${p.id}">${esc(p.name)}</span>`).join('')}</div>` : '';
...
function openBirthModal(preg) {
  ...
  openModal('تسجيل ولادة — ' + display(mother), `
    ...
    document.getElementById('b_save').addEventListener('click', async () => {
      ...
      const base = { type: mother.type, pen_id: mother.pen_id != null ? mother.pen_id : null, source: 'born', status: 'present', mother_id: mother.id, father_name: father, notes };
...
async function quickRevert(a, after) {
  ...
  if (ok) { toast('أُعيدت للحظيرة'); await loadAll(); (after || (() => screenAnimalDetail(String(a.id))))(); }
}
...
function screenSires() {
  ...
  // فحول خارجية أُضيفت قبل وجود حقل النوع (type غائب) تبقى ظاهرة مع كل تصفية احتياطاً حتى تُعدَّل يدوياً
  const extSires = loadExternalSires().filter(s => !animalFilter || !s.type || s.type === animalFilter);
  const canEdit = can('animals', 'edit');
  ...
  const extCard = (s) => `<div class="card" style="margin:6px 0">
      ...
      <div class="li-sub">${s.type ? arOf(TYPES, s.type) : 'نوع غير محدَّد — عدِّله لحصره بقائمة اختيار الأب الصحيحة'}</div>
  ...
  const ae = document.getElementById('s_addext'); if (ae) ae.addEventListener('click', () => externalSireModal(null, animalFilter));
...
function externalSireModal(s, defaultType) {
  openModal(s ? 'تعديل فحل خارج المراح' : 'إضافة فحل خارج المراح', `
    ${fSelect('نوع الحلال', 'exs_type', TYPES, s ? (s.type || '') : (defaultType || (TYPES[0] && TYPES[0].k) || ''))}
    ...
    document.getElementById('exs_save').addEventListener('click', () => {
      ...
      const obj = { type: val('exs_type'), code, name, ownerName: val('exs_owner').trim(), ownerPhone: val('exs_phone').trim(), fatherName: val('exs_father').trim(), notes: val('exs_notes').trim() };
...
function screenFemales() {
  ...
  const card = (a) => {
    ...
    return `<div class="card click" data-aid="${a.id}" style="margin:6px 0">
      <div class="li-title">${display(a)}</div>
      <div class="li-sub">${arOf(TYPES, a.type)}${a.birth ? ' • ' + (ageText(a.birth) || '') : ''}${animalPenName(a) ? ' • 🏠 ' + esc(animalPenName(a)) : ''}</div>
...
function screenNewborns() {
  ...
  // بدل رقم ثابت (٦ أشهر) قد لا يطابق إعداد المستخدم أصلاً، فيبدو المولود «غادر» عند عمر مختلف عمّا تُظهره الشريحة.
  const cfgAges = TYPES.filter(t => !animalFilter || t.k === animalFilter).map(t => countRuleFor(t.k)).filter(r => r.mode === 'age' && r.age > 0).map(r => r.age);
  const lastCut = cfgAges.length ? Math.max(...cfgAges) : 6;
  const ageBucket = (a) => { if (!a.birth) return 'unknown'; const m = ageMonths(a.birth); if (m < 1) return 'lt1'; if (m < 3) return '1to3'; if (m < lastCut) return '3to6'; return 'gt6'; };
  ...
  view().innerHTML = typeChipsHtml() + `<div class="muted" style="margin:8px 0">مواليد لسّه يتبعون أمّهم ولم يُحتسبوا بعد في «في الحظيرة»</div>
    ...
    <div class="chips" style="margin-top:6px">${ageChip('all', 'كل الأعمار')}${ageChip('lt1', 'أقل من شهر')}${ageChip('1to3', '١-٣ أشهر')}${ageChip('3to6', `٣-${lastCut} أشهر`)}${ageChip('gt6', `أكبر من ${lastCut} أشهر`)}</div>
    <div class="muted" style="margin:8px 0">العدد: ${list.length}</div>
    ${list.length ? list.map(card).join('') : noItem()}`;
...
function renderBulkBody() {
  ...
  if (bulkOp === 'buy') {
    ...
    document.getElementById('bk_save').addEventListener('click', async () => {
      ...
      const base = { type: val('bk_type'), pen_id: pen, source: src, status: 'present', color: val('bk_color').trim(),
      ...
      if (ok) { bulkRows.length = 0; lastPen = pen; try { localStorage.setItem('mrahi_last_pen', pen == null ? '' : String(pen)); } catch (e) {} toast(`تم حفظ ${rows.length} رأساً — اضغط أي بهيمة لتعديلها`); bulkSel.clear(); await loadAll(); setHash('#/animals'); }
    });
    renderRows();
    return;
  }
  ...
  const listHtml = cands.length ? cands.map(a => `<label class="bulk-row"><input type="checkbox" data-sel="${a.id}" ${bulkSel.has(a.id) ? 'checked' : ''}><span>${display(a)} <span class="muted">${arOf(TYPES, a.type)}${animalPenName(a) ? ' • ' + esc(animalPenName(a)) : ''}</span></span></label>`).join('') : '<div class="muted">لا توجد بهائم مطابقة.</div>';
...
function quickMenuCats() {
  ...
  return [
    { key: 'breeding', title: '🐑 الحلال والتكاثر', items: [
      ...
      I(can('animals', 'view'), '🗄️ الأرشيف (مباعة/نافقة/مُهداة/مفقودة/مذبوحة)', '#/archive'),
    ].filter(Boolean) },
...
function screenSettingsMenu() {
  ...
  // خريطة التطبيق: بطاقة بارزة أول شيء يظهر — دليل مرئي لكل شيء، لا تختبئ داخل قسم فرعي
  const mapCard = `<div class="card click hl" data-go="#/sitemap" style="background:#e8f5e9;border:1px solid var(--green)"><div class="li-title">🗺️ خريطة التطبيق</div><div class="li-sub">دليل مرئي لكل شيء — الشريط السفلي وكل قوائم ☰/⋮ في صفحة واحدة</div></div>`;
  ...
  renderMenuScreen('settings', cats, mapCard + topUpdate);
...
function screenSiteMap() {
  ...
  view().innerHTML = `<div class="muted" style="margin-bottom:10px">دليل مرئي لكل شيء في حلالي — اضغط أي بند للانتقال إليه مباشرة.</div>
    <div class="card"><h3>📱 الشريط السفلي</h3>
      ...
      ${navStep('🐑', 'الحلال', 'الحلال الموجود حالياً في الحظيرة فقط، بمرشّحات (نوع/مصدر/جنس) — الخارج (بيع/نفوق/اهداء/فقد/ذبح) في 🗄️ الأرشيف', '#/animals')}
      ${navStep('♀️', 'الإناث', 'البالغات سنّ النضج فقط — إحصائيات وفلتر (ملقّحة/معها مواليد/لم تُلقّح)', '#/females')}
      ${navStep('🐏', 'الفحول', 'فحول القطيع باسمها — أبناؤهم وبناتهم والإناث الملقَّحة. وفيها أيضاً قسم «🌍 فحول خارج المراح» وسلسلة النسب', '#/sires')}
...
const aMini = (a) => `<div class="card click" data-aid="${a.id}" style="margin:6px 0"><div class="li-title">${display(a)}</div><div class="li-sub">${arOf(TYPES, a.type)} • ${esc(sexTerm(a))}${animalPenName(a) ? ' • ' + esc(animalPenName(a)) : ''}${a.status !== 'present' ? ' • ' + arOf(STATUS, a.status) : ''}</div></div>`;
...
function renderInspect() {
  ...
  if (inspectTab === 'stats') {
    ...
    const pens = {}; presentT.forEach(a => { const p = animalPenName(a) || '— بلا حظيرة'; pens[p] = (pens[p] || 0) + 1; });
    ...
    const pensById = {}; presentT.forEach(a => { if (a.pen_id != null) pensById[a.pen_id] = (pensById[a.pen_id] || 0) + 1; });
    const penDefsT = allPens().filter(p => !p.type || p.type === TK);
    const rootsWithKids = penDefsT.filter(p => p.parentId == null && penDefsT.some(c => c.parentId === p.id));
    const penTotal = (id) => (pensById[id] || 0) + penDefsT.filter(c => c.parentId === id).reduce((s, c) => s + (pensById[c.id] || 0), 0);
    const hierBoxesHtml = rootsWithKids.length ? `<div class="card"><h3>🏠 حسب الحظيرة (رئيسية وفروعها)</h3>
        <div class="stats" style="grid-template-columns:repeat(${Math.min(rootsWithKids.length, 3)},1fr)">${rootsWithKids.map(r => `<div class="stat"><div class="n">${penTotal(r.id)}</div><div class="l">${esc(r.name)}</div></div>`).join('')}</div>
        ${rootsWithKids.map(r => `<div style="margin-top:10px">${row('🏠 ' + esc(r.name), penTotal(r.id) + ' (إجمالي)')}${penDefsT.filter(c => c.parentId === r.id).sort((a, b) => a.name.localeCompare(b.name, 'ar')).map(c => row('↳ ' + esc(c.name), String(pensById[c.id] || 0))).join('')}</div>`).join('')}
      </div>` : '';
  ...
  if (inspectTab === 'index') {
    ...
    body.innerHTML = `<div class="muted" style="margin:4px 0 8px">فهرس تسلسلي — ${arr.length} رأس في الحظيرة</div>`
      + (arr.length ? arr.map((a, i) => `<div class="card click" data-aid="${a.id}"><div class="li-title">${i + 1}. ${display(a)}</div><div class="li-sub">${arOf(TYPES, a.type)} • ${esc(sexTerm(a))}${animalPenName(a) ? ' • ' + esc(animalPenName(a)) : ''}</div></div>`).join('') : noItem());
  ...
  if (inspectTab === 'offspring') {
    ...
    body.innerHTML = `<div class="muted" style="margin:4px 0 8px">الأمهات حسب عدد المواليد المسجّلة (${moms.length} أم منتِجة)</div>`
      + (moms.length ? moms.map(({ a, n }) => `<div class="card click" data-aid="${a.id}"><div class="li-title">${display(a)}</div><div class="li-sub">👶 ${n} مولود • ${arOf(TYPES, a.type)}${animalPenName(a) ? ' • ' + esc(animalPenName(a)) : ''}</div></div>`).join('') : noItem());
    bindCards(body); return;
  }
  if (inspectTab === 'agefilter') {
    ...
    const compute = () => {
      ...
      res.innerHTML = `<div class="card hl"><div class="li-title">النتيجة: ${arr.length} رأس</div><div class="li-sub">${afSex === 'male' ? 'ذكور' : afSex === 'female' ? 'إناث' : 'الكل'} • ${afSrc === 'born' ? 'مواليد' : afSrc === 'purchased' ? 'مشترى' : 'كل المصادر'} • ${afCmp === 'gt' ? 'أكبر من' : 'أصغر من'} ${afMonths} شهر${noB ? ` • (${noB} بلا ميلاد غير محسوبة)` : ''}</div></div>`
        + (arr.length ? arr.map(a => `<div class="card click" data-aid="${a.id}"><div class="li-title">${display(a)} <span class="muted" style="font-weight:400">🎂 ${ageText(a.birth)}</span></div><div class="li-sub">${arOf(TYPES, a.type)} • ${esc(sexTerm(a))}${animalPenName(a) ? ' • ' + esc(animalPenName(a)) : ''}</div></div>`).join('') : noItem());
      bindCards(res);
    };
...
function screenHerdSettings() {
  ...
  const groups = [
    ...
    { h: '👶 متابعة الولادات', items: [
      ['📅', 'عمر حدّ البلوغ (احتساب المولود)، والذكور/الفحول، ونقل الحظيرة عند البلوغ', '#/countage'],
    ] },
...
// عمر حدّ البلوغ (احتساب المولود) في الحظيرة (لكل نوع + لأي جنس تنطبق القاعدة) + حظيرة الوجهة عند بلوغه
function screenCountAge() {
  ...
  view().innerHTML = `
    ...
    <div class="card"><h3>ظهور المواليد غير المحتسَبة</h3>
      <div class="muted" style="font-size:.82rem;margin-bottom:8px">المولود الأصغر من عمر حدّ البلوغ أدناه «يتبع أمّه» ولا يُحتسب في «في الحظيرة». هذا الخيار يتحكّم هل يظهر في قائمة «الحلال» أيضاً أثناء ذلك.</div>
    ...
    <div class="muted" style="margin-bottom:8px">لكل نوع: <b>عمر حدّ البلوغ</b> — متى يُحتسب المولود ضمن «في الحظيرة» (<b>عند عمر</b>: تلقائياً عند بلوغه، <b>يدوي</b>: لا يُحتسب حتى تضيفه بنفسك). أصغر من ذلك «يتبع أمّه». المشترى/الاهداء يُحتسب دائماً. <b>حظيرة الوجهة</b> (اختياري) — إن حُدّدت، ستظهر لك البهيمة في 🔔 التنبيهات مقترَحاً نقلها إليها فور بلوغها هذا العمر (أو فور ضبط غرض الذكر «معدّ للبيع»)، وتنقلها بنفسك بضغطة تأكيد — لا نقل صامت.</div>
    ${TYPES.map(t => {
      const r = countRuleFor(t.k);
      ensureSalePen(t.k);   // تضمن وجود «💰 معدّ للبيع» كخيار جاهز دائماً في قائمتَي حظيرة الوجهة أدناه
      return `<div class="card"><h3>${esc(t.ar)}</h3>${fSelect('طريقة الاحتساب', 'cm_' + t.k, MODES, r.mode)}<div id="cab_${t.k}">${fInput('العمر (أشهر) — صفر = يُحتسب الجميع', 'ca_' + t.k, r.age || '', 'number', 'min="0" inputmode="numeric"')}</div>${fSelect('تنطبق على', 'cas_' + t.k, SEXSCOPE, r.sex)}
        <div class="field"><label>🐏 حظيرة وجهة الذكور (عند بلوغ العمر، أو عند ضبط غرضهم «معدّ للبيع»)</label>${penField('pm_' + t.k, r.malePenId, t.k)}</div>
        <div id="pf_box_${t.k}"><div class="field"><label>♀️ حظيرة وجهة الإناث (عند بلوغ العمر)</label>${penField('pf_' + t.k, r.femalePenId, t.k)}</div></div>
      </div>`;
    }).join('')}
    <button class="btn" id="ca_save">حفظ</button>`;
  // حقل العمر (وحظيرة وجهة الإناث المرتبطة به) يظهران فقط في وضع «عند عمر» — وجهة الذكور تبقى متاحة دائماً (تنطبق أيضاً عبر غرض «معدّ للبيع» بصرف النظر عن وضع الاحتساب)
  TYPES.forEach(t => {
    const sel = document.getElementById('cm_' + t.k); const box = document.getElementById('cab_' + t.k); const pfBox = document.getElementById('pf_box_' + t.k);
    if (!sel) return;
    const sync = () => { const isAge = sel.value === 'age'; if (box) box.style.display = isAge ? '' : 'none'; if (pfBox) pfBox.style.display = isAge ? '' : 'none'; };
    sel.addEventListener('change', sync); sync();
    bindPenField('pm_' + t.k); bindPenField('pf_' + t.k);
  });
  document.getElementById('ca_save').addEventListener('click', () => {
    const o = {};
    TYPES.forEach(t => {
      const mode = val('cm_' + t.k) === 'manual' ? 'manual' : 'age';
      const n = parseInt(val('ca_' + t.k), 10) || 0;
      const sex = val('cas_' + t.k) || 'both';
      const malePenId = penValue('pm_' + t.k, t.k);
      const femalePenId = mode === 'age' ? penValue('pf_' + t.k, t.k) : null;
      const base = { sex, malePenId, femalePenId };
      if (mode === 'manual') o[t.k] = Object.assign({ mode: 'manual', age: 0 }, base);
      else if (n > 0 || malePenId != null || femalePenId != null) o[t.k] = Object.assign({ mode: 'age', age: n }, base);
    });
    saveCountAge(o);
    try {
...
function screenPens() {
  ...
  TYPES.map(t => t.k).concat(['']).forEach(tk => {
    ...
    const roots = arr.filter(p => p.parentId == null).sort((a, b) => a.name.localeCompare(b.name, 'ar'));
    const orphanBranches = arr.filter(p => p.parentId != null && !arr.some(q => q.id === p.parentId));   // أب من مجموعة أخرى أو محذوف — تُعرض مستقلة احتياطاً
    let html = '';
    roots.forEach(p => {
  ...
  view().innerHTML = `<div class="card" style="background:#fff8e1">
      ...
      <div class="muted" style="font-size:.8rem;margin-top:4px">الحظيرة مجرّد تصنيف مكاني — لا تُخفي البهيمة عن تبويباتها الأخرى: الأنثى تبقى تظهر في ♀️ الإناث، المولود في 👶 المواليد، الفحل في 🐏 الفحول، وتُحتسب في «في الحظيرة» بالرئيسية كالمعتاد. البهيمة مرتبطة برقم الحظيرة الداخلي الثابت لا باسمها — فتعديل الاسم يظهر فوراً في كل مكان.</div>
    </div>
  ...
  view().querySelectorAll('[data-penroot]').forEach(el => el.addEventListener('click', (e) => {
    ...
    penRootModal(parseInt(el.dataset.penroot, 10));
  }));
  view().querySelectorAll('[data-penedit]').forEach(b => b.addEventListener('click', (e) => { e.stopPropagation(); penRenameModal(parseInt(b.dataset.penedit, 10)); }));
  view().querySelectorAll('[data-pendel]').forEach(b => b.addEventListener('click', (e) => { e.stopPropagation(); penDelete(parseInt(b.dataset.pendel, 10)); }));
  view().querySelectorAll('[data-penmove]').forEach(b => b.addEventListener('click', (e) => { e.stopPropagation(); penMoveModal(parseInt(b.dataset.penmove, 10)); }));
  view().querySelectorAll('[data-penadd]').forEach(b => b.addEventListener('click', (e) => { e.stopPropagation(); penAssignModal(parseInt(b.dataset.penadd, 10)); }));
}
...
function penAssignModal(penId) {
  const pen = penById(penId); if (!pen) return;
  const candidates = (C.animals || []).filter(a => a.status === 'present' && a.pen_id !== penId && (!pen.type || a.type === pen.type)).sort((a, b) => animalPenName(a).localeCompare(animalPenName(b), 'ar'));
  if (!candidates.length) { toast('لا توجد بهائم أخرى (من نفس النوع) يمكن إضافتها'); return; }
  const rows = candidates.map(a => `<label class="bulk-row"><input type="checkbox" data-pa="${a.id}"><span>${display(a)} <span class="muted">${arOf(TYPES, a.type)} • ${animalPenName(a) ? esc(animalPenName(a)) : 'بلا حظيرة'}</span></span></label>`).join('');
  openModal('➕ إضافة بهائم إلى 🏠 ' + pen.name, `
    ...
    document.getElementById('pa_go').addEventListener('click', async () => {
      ...
      if (!ids.length) { toast('اختر بهيمة واحدة على الأقل'); return; }
      if (!await confirm2(`إضافة ${ids.length} بهيمة إلى «${pen.name}»؟`)) return;
      const ok = await guard(async () => { for (const id of ids) await dbUpdate('animals', id, { pen_id: penId }); });
...
function penRootModal(id) {
  const cur = penById(id); if (!cur) return;
  ...
  openModal('🏠 ' + cur.name, `
    ...
    document.getElementById('pb_add').addEventListener('click', async () => {
      ...
      addPen(n, cur.type, id);
      toast('أُضيف الفرع'); closeModal(); screenPens();
    });
  });
}
// نقل كل بهائم حظيرة (أو المحدَّد منها) إلى حظيرة أخرى — نقل كامل: pen_id يُستبدَل بالكامل، لا تبقى أي علاقة بالحظيرة السابقة
function penMoveModal(fromId) {
  const fromPen = penById(fromId); if (!fromPen) return;
  const animals = (C.animals || []).filter(a => a.pen_id === fromId && a.status === 'present');
  if (!animals.length) { toast('لا توجد بهائم في هذه الحظيرة'); return; }
  const destOpts = allPens().filter(p => p.id !== fromId).map(p => `<option value="${p.id}">${p.parentId != null ? esc((penById(p.parentId) || {}).name || '؟') + ' ↳ ' + esc(p.name) : '🏠 ' + esc(p.name)}</option>`).join('');
  ...
  openModal('نقل من 🏠 ' + fromPen.name, `
    ...
    document.getElementById('pm_go').addEventListener('click', async () => {
      const destRaw = val('pm_dest'); if (!destRaw) { toast('اختر الحظيرة الجديدة'); return; }
      const destId = parseInt(destRaw, 10);
      const destPen = penById(destId);
      const ids = [...document.querySelectorAll('[data-mv]:checked')].map(c => parseInt(c.dataset.mv, 10));
      if (!ids.length) { toast('اختر بهيمة واحدة على الأقل'); return; }
      if (!await confirm2(`نقل ${ids.length} بهيمة من «${fromPen.name}» إلى «${destPen ? destPen.name : ''}»؟ نقل كامل — لن تبقى في «${fromPen.name}».`, { danger: true })) return;
      const ok = await guard(async () => { for (const id of ids) await dbUpdate('animals', id, { pen_id: destId }); });
...
async function penDelete(id) {
  ...
  if (!await confirm2(`حذف «${pen.name}» من القائمة؟`)) return;
  savePens(loadPens().filter(p => p.id !== id));
...
function remAnimalRows(typeKey, selected) {
  ...
  return list.length ? list.map(a => {
    ...
    const extra = [esc(sexTerm(a)), a.birth ? 'مواليد ' + fmtDate(a.birth) : '', mom ? '🤱 أم: ' + display(mom) : '', animalPenName(a) ? '🏠 ' + esc(animalPenName(a)) : ''].filter(Boolean).join(' • ');
